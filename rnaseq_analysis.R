...
# do genome scans 
out <- scan1(genoprobs, expr.mrna, K, addcovar=covar, cores=0)

# save output to RDS file
saveRDS(out, "rnaseq_output.rds")
...
scan_batch <- function(batch) {
    ...
    scan1(genoprobs, expr.mrna[,batch,drop=FALSE], K, addcovar=cbind(covar, pr_covar[rownames(covar),-1]), cores=1)
}
...
saveRDS(out_vlocal, "rnaseq_output_vlocal.rds")
